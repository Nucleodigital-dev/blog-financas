...
const categoryNameOverrides: Record<string, Pick<Category, "name_pt" | "name_en">> = {
  "financas-pessoais": { name_pt: "Finanças Pessoais", name_en: "Personal Finance" },
  "investimentos": { name_pt: "Investimentos", name_en: "Investments" },
  "renda-extra": { name_pt: "Renda Extra", name_en: "Side Income" },
  "planejamento-financeiro": { name_pt: "Planejamento Financeiro", name_en: "Financial Planning" },
};
...
export async function getAllArticles(): Promise<Article[]> {
  ...
  try {
    const { data, error } = await supabase
      .from("articles")
      .select(
        "id, slug, title_pt, title_en, seo_description, content_pt, content_en, cover_image, cover_alt, category_id, is_featured, status, created_at, published_at"
      )
      .eq("status", "published")
