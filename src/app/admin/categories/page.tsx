    ...
    try {
      if (formData.config_json) {
        try {
          ...
          alert("O JSON de configuração é inválido.");
          setSaving(false);
          return;
        }
      }
  ...
  return (
    <div className="admin-editor-page" style={{ maxWidth: 900, margin: '0 auto', paddingTop: 40, paddingBottom: 60 }}>
      <div className="admin-header">
        <div>
          <h1 style={{ fontFamily: 'var(--font-heading)' }}>Categorias</h1>
          <p style={{ color: 'var(--text-muted)', marginTop: 4 }}>Gerencie as categorias de finanças</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
        <form className="admin-card" onSubmit={handleSubmit} style={{ background: 'var(--card-bg)', padding: 24, borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
          <h2 style={{ marginBottom: 24, fontFamily: 'var(--font-heading)' }}>Nova Categoria</h2>
          
          <div className="form-group">
            <label>Nome (PT-BR)</label>
            <div style={{ display: 'flex', gap: 8 }}>
              <input required value={formData.name_pt} onChange={handleNameChange} placeholder="Ex: Investimentos" />
              <button type="button" onClick={handleTranslate} className="btn btn-secondary" style={{ padding: '0 16px', whiteSpace: 'nowrap' }}>Traduzir</button>
            </div>
          </div>

          <div className="form-group">
            <label>Nome (EN-US)</label>
            <input value={formData.name_en} onChange={e => setFormData({...formData, name_en: e.target.value})} placeholder="Ex: Investments" />
          </div>

          <div className="form-group">
            <label>Slug</label>
            <input required value={formData.slug} onChange={e => setFormData({...formData, slug: e.target.value})} placeholder="ex: investimentos" />
          </div>
          ...
          <div className="form-group">
            ...
            <textarea 
               ...
               placeholder={`{\n  "required": ["intro", "concepts"],\n  "optional": ["comparison_table"]\n}`}
               style={{ minHeight: 120, fontFamily: 'monospace', fontSize: '0.85rem' }}
            />
            <small style={{ color: 'var(--text-muted)' }}>Personalize os blocos disponíveis para esta categoria.</small>
          </div>
        ...
        <div className="admin-card" style={{ background: 'var(--card-bg)', padding: 24, borderRadius: 'var(--radius)', border: '1px solid var(--border)' }}>
          <h2 style={{ marginBottom: 24, fontFamily: 'var(--font-heading)' }}>Categorias Existentes</h2>
          {loading ? <p>Carregando...</p> : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {categories.map(c => {
                const parent = categories.find(p => p.id === c.parent_id);
                return (
                  <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 16, border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', background: 'var(--background)' }}>
                    <div>
                      <strong>{c.name_pt}</strong> <span style={{ color: 'var(--text-muted)' }}>({c.name_en})</span>
                      <div style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>
                        /{c.slug} {parent && ` • ${parent.name_pt}`}
                      </div>
                    </div>
