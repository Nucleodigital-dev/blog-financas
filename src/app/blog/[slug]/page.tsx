...
const SectionHeader = ({ preTitle, title }: { preTitle: string; title: string }) => (
  <div style={{ margin: "56px 0 32px 0" }}>
    ...
    <h2 style={{ margin: 0, fontSize: "2.35rem", lineHeight: 1.25, color: "var(--foreground)", fontFamily: "var(--font-heading)" }}>
      {title}
    </h2>
  </div>
);
  ...
  return (
    <>
      <style
        ...
        .block-exercise_routine ul, .block-checklist ul {
          display: flex;
          flex-direction: column;
          gap: 16px;
          padding: 0;
        }
        .block-exercise_routine li, .block-checklist li {
          background: var(--card-bg);
          padding: 24px;
          border-radius: 12px;
      ...
      <article lang={isEn ? "en" : "pt-BR"} style={{ maxWidth: 860, margin: "0 auto", paddingTop: 40, paddingBottom: 80 }}>
        ...
        <div style={{ marginBottom: 40 }}>
          ...
          >
            ...
            {catName && (
              <>
                <Link href={`/?category=${article.cat_slug || article.category_id}`} style={{ color: "var(--primary)", fontWeight: 600 }}>
                  {catName}
                </Link>
                <ChevronRight size={14} />
              </>
            ...
            >
              <BookOpen size={14} /> {copy.educational}
            </span>
          </div>
        ...
        {isJsonBlocks && quickAnswerBlock && (
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20, color: "var(--primary)" }}>
              <Info size={28} />
              <h3 style={{ margin: 0, fontSize: "1.5rem", fontWeight: 800, fontFamily: "var(--font-heading)" }}>{copy.summary}</h3>
            </div>
            <div
              dangerouslySetInnerHTML={{
        ...
        <div className="article-content">
          ...
          {["calculadora-juros-compostos", "simulador-reserva-emergencia", "calculadora-cet", "simulador-independencia-financeira", "comparador-renda-fixa", "financas-pessoais-como-organizar-seu-dinheiro", "planejador-reserva-impostos", "orcamento-apertado-priorizar-gastos", "pacote-bancario-ou-servicos-avulsos", "renda-variavel-limite-exposicao-risco", "mei-controle-faturamento-limite-anual"].includes(slug) && (
            ...
            <div style={{ display: "flex", flexDirection: "column" }}>
              ...
              {originalBlocks.length > 0 && otherBlocks.length > 0 && (
                <div style={{ background: "var(--card-bg)", padding: 32, borderRadius: 20, border: "1px solid var(--border)", marginBottom: 56 }}>
                  <p style={{ fontWeight: 800, marginBottom: 20, fontSize: "1.25rem", fontFamily: "var(--font-heading)" }}>{copy.toc}</p>
                  <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 16 }}>
                    {otherBlocks.map((b, i) => (
                      <li key={i} style={{ margin: 0 }}>
                        ...
                        >
                          <span style={{ color: "var(--text-muted)", fontSize: "1rem", fontWeight: 600 }}>0{i + 1}.</span>{" "}
              ...
              {otherBlocks.map((block: any, i: number) => {
                  ...
                  return (
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20, color: "#ef4444" }}>
                        ...
                        <h2 style={{ fontSize: "1.75rem", margin: 0, fontFamily: "var(--font-heading)", color: "#ef4444" }}>{block.title}</h2>
                      </div>
                      <div
                  ...
                  return (
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20, color: "var(--primary)" }}>
                        ...
                        <h2 style={{ fontSize: "1.75rem", margin: 0, fontFamily: "var(--font-heading)", color: "var(--primary)" }}>{block.title}</h2>
                      </div>
                      <div
                        className={`block-${block.type}`}
                        dangerouslySetInnerHTML={{
                ...
                if (block.type === "faq" && block.faqs) {
                  return (
                    <div key={i} id={sectionId} style={{ marginBottom: 48 }}>
                      <SectionHeader preTitle={copy.faq} title={block.title} />
                      <div style={{ display: "flex", flexDirection: "column" }}>
                ...
                let preTitle = copy.preTitle.default;
                if (block.type === "intro") preTitle = copy.preTitle.intro;
                if (block.type === "causes") preTitle = copy.preTitle.causes;
                if (block.type === "exercise_routine") preTitle = copy.preTitle.exercise_routine;
                if (block.type === "references") preTitle = copy.preTitle.references;
        ...
        <div className="related-articles">
          <h3 style={{ fontSize: "1.75rem", marginBottom: 32, color: "var(--foreground)", fontFamily: "var(--font-heading)" }}>
            {copy.related}
          </h3>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ padding: 24 }}>
                    <h4 style={{ fontSize: "1.25rem", margin: "0 0 8px 0", color: "var(--foreground)", fontFamily: "var(--font-heading)" }}>
                      {relTitle}
                    </h4>
                    <p
                      style={{
                    ...
                    >
                      {copy.readArticle} <ChevronRight size={16} />
                    </p>
                  </div>
                </Link>
              );
